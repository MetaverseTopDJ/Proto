// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.26.0
// 	protoc        v3.19.1
// source: nft/nft.proto

package pb

import (
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

var File_nft_nft_proto protoreflect.FileDescriptor

var file_nft_nft_proto_rawDesc = []byte{
	,
}

var file_nft_nft_proto_goTypes = []interface{}{
	(*InfoPost)(nil),                     // 0: global.InfoPost
	(*EmptyPost)(nil),                    // 1: global.EmptyPost
	(*PaginationPost)(nil),               // 2: global.PaginationPost
	(*CreateAuthorPost)(nil),             // 3: nft.CreateAuthorPost
	(*UpdateAuthorPost)(nil),             // 4: nft.UpdateAuthorPost
	(*ChangeStatusPost)(nil),             // 5: global.ChangeStatusPost
	(*RecommendWorksPost)(nil),           // 6: nft.RecommendWorksPost
	(*AuthorWorkPaginationPost)(nil),     // 7: nft.AuthorWorkPaginationPost
	(*CreateWorkPost)(nil),               // 8: nft.CreateWorkPost
	(*UpdateWorkPost)(nil),               // 9: nft.UpdateWorkPost
	(*CreateBlindBoxPost)(nil),           // 10: nft.CreateBlindBoxPost
	(*UpdateBlindBoxPost)(nil),           // 11: nft.UpdateBlindBoxPost
	(*CreateContractPost)(nil),           // 12: global.CreateContractPost
	(*UpdateContractPost)(nil),           // 13: global.UpdateContractPost
	(*CheckBlindBoxTxPost)(nil),          // 14: nft.CheckBlindBoxTxPost
	(*AuthorResponse)(nil),               // 15: nft.AuthorResponse
	(*SimpleAuthorResponse)(nil),         // 16: nft.SimpleAuthorResponse
	(*HotAuthorsResponse)(nil),           // 17: nft.HotAuthorsResponse
	(*AuthorPaginationResponse)(nil),     // 18: nft.AuthorPaginationResponse
	(*WorkResponse)(nil),                 // 19: nft.WorkResponse
	(*WorkDetailResponse)(nil),           // 20: nft.WorkDetailResponse
	(*RecommendWorksResponse)(nil),       // 21: nft.RecommendWorksResponse
	(*AuthorWorkPaginationResponse)(nil), // 22: nft.AuthorWorkPaginationResponse
	(*WorkPaginationResponse)(nil),       // 23: nft.WorkPaginationResponse
	(*BlindBoxResponse)(nil),             // 24: nft.BlindBoxResponse
	(*BlindBoxCountResponse)(nil),        // 25: nft.BlindBoxCountResponse
	(*BlindBoxTopResponse)(nil),          // 26: nft.BlindBoxTopResponse
	(*BlindBoxPaginationResponse)(nil),   // 27: nft.BlindBoxPaginationResponse
	(*ContractResponse)(nil),             // 28: global.ContractResponse
	(*ContractsResponse)(nil),            // 29: global.ContractsResponse
	(*ContractAddressResponse)(nil),      // 30: global.ContractAddressResponse
	(*ContractPaginationResponse)(nil),   // 31: global.ContractPaginationResponse
	(*CheckResponse)(nil),                // 32: global.CheckResponse
}
var file_nft_nft_proto_depIdxs = []int32{
	0,  // 0: nft.NFTService.Author:input_type -> global.InfoPost
	0,  // 1: nft.NFTService.AuthorDetail:input_type -> global.InfoPost
	1,  // 2: nft.NFTService.HotAuthors:input_type -> global.EmptyPost
	1,  // 3: nft.NFTService.CacheAllAuthors:input_type -> global.EmptyPost
	2,  // 4: nft.NFTService.AuthorPagination:input_type -> global.PaginationPost
	3,  // 5: nft.NFTService.CreateAuthor:input_type -> nft.CreateAuthorPost
	4,  // 6: nft.NFTService.UpdateAuthor:input_type -> nft.UpdateAuthorPost
	5,  // 7: nft.NFTService.ChangeAuthorStatus:input_type -> global.ChangeStatusPost
	0,  // 8: nft.NFTService.Work:input_type -> global.InfoPost
	0,  // 9: nft.NFTService.WorkDetail:input_type -> global.InfoPost
	6,  // 10: nft.NFTService.RecommendWorks:input_type -> nft.RecommendWorksPost
	7,  // 11: nft.NFTService.AuthorWorkPagination:input_type -> nft.AuthorWorkPaginationPost
	2,  // 12: nft.NFTService.WorkPagination:input_type -> global.PaginationPost
	8,  // 13: nft.NFTService.CreateWork:input_type -> nft.CreateWorkPost
	9,  // 14: nft.NFTService.UpdateWork:input_type -> nft.UpdateWorkPost
	5,  // 15: nft.NFTService.ChangeWorkStatus:input_type -> global.ChangeStatusPost
	0,  // 16: nft.NFTService.BlindBox:input_type -> global.InfoPost
	0,  // 17: nft.NFTService.BlindBoxCount:input_type -> global.InfoPost
	1,  // 18: nft.NFTService.BlindBoxTop:input_type -> global.EmptyPost
	1,  // 19: nft.NFTService.BlindBoxTopCount:input_type -> global.EmptyPost
	2,  // 20: nft.NFTService.BlindBoxPagination:input_type -> global.PaginationPost
	10, // 21: nft.NFTService.CreateBlindBox:input_type -> nft.CreateBlindBoxPost
	11, // 22: nft.NFTService.UpdateBlindBox:input_type -> nft.UpdateBlindBoxPost
	5,  // 23: nft.NFTService.StartBlindBox:input_type -> global.ChangeStatusPost
	0,  // 24: nft.NFTService.NftContract:input_type -> global.InfoPost
	1,  // 25: nft.NFTService.NftContracts:input_type -> global.EmptyPost
	1,  // 26: nft.NFTService.NftContractAddress:input_type -> global.EmptyPost
	2,  // 27: nft.NFTService.NftContractPagination:input_type -> global.PaginationPost
	12, // 28: nft.NFTService.CreateNftContract:input_type -> global.CreateContractPost
	13, // 29: nft.NFTService.UpdateNftContract:input_type -> global.UpdateContractPost
	5,  // 30: nft.NFTService.ChangeNftContractStatus:input_type -> global.ChangeStatusPost
	14, // 31: nft.NFTService.CheckBlindBoxTx:input_type -> nft.CheckBlindBoxTxPost
	15, // 32: nft.NFTService.Author:output_type -> nft.AuthorResponse
	16, // 33: nft.NFTService.AuthorDetail:output_type -> nft.SimpleAuthorResponse
	17, // 34: nft.NFTService.HotAuthors:output_type -> nft.HotAuthorsResponse
	17, // 35: nft.NFTService.CacheAllAuthors:output_type -> nft.HotAuthorsResponse
	18, // 36: nft.NFTService.AuthorPagination:output_type -> nft.AuthorPaginationResponse
	15, // 37: nft.NFTService.CreateAuthor:output_type -> nft.AuthorResponse
	15, // 38: nft.NFTService.UpdateAuthor:output_type -> nft.AuthorResponse
	15, // 39: nft.NFTService.ChangeAuthorStatus:output_type -> nft.AuthorResponse
	19, // 40: nft.NFTService.Work:output_type -> nft.WorkResponse
	20, // 41: nft.NFTService.WorkDetail:output_type -> nft.WorkDetailResponse
	21, // 42: nft.NFTService.RecommendWorks:output_type -> nft.RecommendWorksResponse
	22, // 43: nft.NFTService.AuthorWorkPagination:output_type -> nft.AuthorWorkPaginationResponse
	23, // 44: nft.NFTService.WorkPagination:output_type -> nft.WorkPaginationResponse
	19, // 45: nft.NFTService.CreateWork:output_type -> nft.WorkResponse
	19, // 46: nft.NFTService.UpdateWork:output_type -> nft.WorkResponse
	19, // 47: nft.NFTService.ChangeWorkStatus:output_type -> nft.WorkResponse
	24, // 48: nft.NFTService.BlindBox:output_type -> nft.BlindBoxResponse
	25, // 49: nft.NFTService.BlindBoxCount:output_type -> nft.BlindBoxCountResponse
	26, // 50: nft.NFTService.BlindBoxTop:output_type -> nft.BlindBoxTopResponse
	25, // 51: nft.NFTService.BlindBoxTopCount:output_type -> nft.BlindBoxCountResponse
	27, // 52: nft.NFTService.BlindBoxPagination:output_type -> nft.BlindBoxPaginationResponse
	24, // 53: nft.NFTService.CreateBlindBox:output_type -> nft.BlindBoxResponse
	24, // 54: nft.NFTService.UpdateBlindBox:output_type -> nft.BlindBoxResponse
	24, // 55: nft.NFTService.StartBlindBox:output_type -> nft.BlindBoxResponse
	28, // 56: nft.NFTService.NftContract:output_type -> global.ContractResponse
	29, // 57: nft.NFTService.NftContracts:output_type -> global.ContractsResponse
	30, // 58: nft.NFTService.NftContractAddress:output_type -> global.ContractAddressResponse
	31, // 59: nft.NFTService.NftContractPagination:output_type -> global.ContractPaginationResponse
	28, // 60: nft.NFTService.CreateNftContract:output_type -> global.ContractResponse
	28, // 61: nft.NFTService.UpdateNftContract:output_type -> global.ContractResponse
	28, // 62: nft.NFTService.ChangeNftContractStatus:output_type -> global.ContractResponse
	32, // 63: nft.NFTService.CheckBlindBoxTx:output_type -> global.CheckResponse
	32, // [32:64] is the sub-list for method output_type
	0,  // [0:32] is the sub-list for method input_type
	0,  // [0:0] is the sub-list for extension type_name
	0,  // [0:0] is the sub-list for extension extendee
	0,  // [0:0] is the sub-list for field type_name
}

func init() { file_nft_nft_proto_init() }
func file_nft_nft_proto_init() {
	if File_nft_nft_proto != nil {
		return
	}
	file_global_global_proto_init()
	file_global_contract_proto_init()
	file_nft_author_proto_init()
	file_nft_work_proto_init()
	file_nft_blind_box_proto_init()
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: file_nft_nft_proto_rawDesc,
			NumEnums:      0,
			NumMessages:   0,
			NumExtensions: 0,
			NumServices:   1,
		},
		GoTypes:           file_nft_nft_proto_goTypes,
		DependencyIndexes: file_nft_nft_proto_depIdxs,
	}.Build()
	File_nft_nft_proto = out.File
	file_nft_nft_proto_rawDesc = nil
	file_nft_nft_proto_goTypes = nil
	file_nft_nft_proto_depIdxs = nil
}
